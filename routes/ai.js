...
router.post("/createCourse", authenticateToken, async function (req, res, next) {
...
router.get("/genMCQ", authenticateToken, async function (req, res, next) {
  ...
  var apiResponse = await apiWrapper.gen_mcq(req.query.argomento, req.query.nr_domande);
...
router.get("/genOQ", authenticateToken,  async function (req, res, next) {
  ...
  var apiResponse = await apiWrapper.gen_oq(req.query.argomento, req.query.nr_domande);
  res.send(apiResponse);
});
...
router.post("/explain", authenticateToken, upload.single("appunti"), async function (req, res, next) {
  const fileBuffer = req.file.buffer;
  
  var resp = await apiWrapper.explain(req.user, fileBuffer, req.body.highlight);
...
router.post("/chatCourse", authenticateToken, async function (req, res, next) {
  ...
  var resp = await apiWrapper.chatCourse(req.user, req.body.course_id, req.body.domanda);
